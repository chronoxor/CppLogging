...
public:
    ...
    */
    explicit AsyncWaitProcessor(const std::shared_ptr<Layout>& layout, bool autostart = true, size_t capacity = 8192, size_t initial = 8192, const std::function<void ()>& on_thread_initialize = [](){}, const std::function<void ()>& on_thread_clenup = [](){});
